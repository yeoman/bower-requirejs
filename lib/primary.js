...
module.exports = function (name, dep) {
  ...
  function findByDirname() {
    if (_.contains(main, path.basename(dep.canonicalDir) + '.js')) {
      main = [path.basename(dep.canonicalDir) + '.js'];
    }
  }
  ...
  function findByDirnameSuffix() {
    if (_.contains(main, path.basename(dep.canonicalDir).replace(/js$/, '') + '.js')) {
      main = [path.basename(dep.canonicalDir).replace(/js$/, '') + '.js'];
    }
  }
  ...
  var main = file.expand({ cwd: dep.canonicalDir }, '*.js', '!*.min.js');
